...
def make_manifest(
    resource: str = "sessions", *, mode: ConnectorMode = ConnectorMode.SIMULATOR, **overrides: object
) -> ConnectorManifest:
...
def test_each_approved_resource_has_a_tenant_scoped_read_only_manifest(resource: str) -> None:
...
def test_evidence_response_preserves_declared_failure_state(failure_state: ConnectorFailureState) -> None:
...
def test_evidence_request_is_case_and_tenant_scoped_with_bounded_read_parameters() -> None:
...
def test_manifest_limits_are_explicit_and_connector_statuses_are_closed_set() -> None:
    limits = ConnectorLimits(max_page_size=250, timeout_seconds=30, max_payload_bytes=4096)
